    ...
    <section className={`${WB_SECTION} relative overflow-hidden bg-white`}>
      <div className={WB_WRAP}>
        ...
        <div className="relative overflow-hidden">
          <div
            className="flex w-max animate-[marquee-left_40s_linear_infinite] gap-4 py-3 pb-6 hover:[animation-play-state:paused] sm:gap-5 sm:pb-7"
          >
            {[...videoReviews, ...videoReviews].map((review, index) => (
              <motion.div
                key={`${review.id}-${index}`}
                className="min-w-[220px] shrink-0 cursor-pointer overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 sm:min-w-[240px]"
                whileHover={{ y: -6 }}
