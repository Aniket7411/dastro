...
import { formatTime, getTargetTimestamp } from '../face-reading/FixedBottomCTA';
  ...
  return (
    <ModalPortal open={isOpen}>
      <ModalOverlay onClose={onClose} className="!items-center !p-3 sm:!p-4">
        <div className="relative mx-2 flex max-h-[calc(100vh-1rem)] w-full max-w-4xl flex-col overflow-hidden rounded-[14px] bg-white shadow-[0_30px_80px_rgba(15,23,42,0.34)] ring-1 ring-white/15 sm:mx-4" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="webinar-modal-title">
          <button type="button" className="absolute right-3 top-14 z-[100] flex h-9 w-9 items-center justify-center rounded-full border border-white/70 bg-white/95 text-slate-900 shadow-[0_10px_24px_rgba(15,23,42,0.22)] backdrop-blur transition hover:scale-105 hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EE6662] sm:right-4 sm:top-11 sm:h-10 sm:w-10" onClick={onClose} aria-label="Close">
          ...
          <div className="flex h-full min-h-0 flex-col overflow-hidden">
            <div className="grid w-full grid-cols-1 md:grid-cols-[0.82fr_1.18fr] md:min-h-[252px] lg:min-h-[280px]">
              <div className="relative mt-2 h-[275px] w-full overflow-hidden rounded-t-[6px] bg-white p-2 sm:mt-0 sm:h-[310px] sm:p-3 md:order-2 md:h-auto md:rounded-t-none md:rounded-tr-[14px]">
                <img src="/facereading/facereadingreview.webp" alt="Damini Shukla" className="h-full w-full rounded-[14px] object-contain object-center transition-transform duration-1000" />
              </div>
              ...
              <div className="hidden flex-col justify-center border-t border-white/10 bg-[#3B2261] px-5 py-4 text-white md:order-1 md:flex md:border-r md:border-t-0 md:border-white/10 md:px-6 md:py-5 lg:px-7 lg:py-5">
                ...
                <h4 className={`${TYPE.h2} !m-0 !text-white !leading-[1.08] text-[21px] drop-shadow-md sm:text-[24px] md:text-[28px] lg:text-[31px]`}>
                  2-Day Vedic{' '}
                  <span className={`${WB_HIGHLIGHT} ml-1 inline-block`}>Numerology Masterclass</span>
                </h4>
              </div>
            </div>
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-5">
                <div className="mb-3 rounded-[18px] border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
                  <h3 id="webinar-modal-title" className={`${TYPE.h3} mb-2 flex items-center gap-2 text-[18px] text-slate-800 sm:text-[20px]`}>
                    ...
                    2-Day Vedic Numerology Masterclass
                  </h3>
                ...
                <div className="mb-3">
                  <ul className="m-0 grid gap-1.5 p-0 sm:grid-cols-3">
                    {[
                      ...
                      'Learn the 4-Number Method + Lo Shu Grid',
                      'Na chart, na deck - sirf aapki date of birth',
                    ].map((point) => (
                ...
                <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-2">
                  <div>
                    ...
                    <input id="webinar-name" type="text" name="name" value={formData.name} onChange={handleChange} required autoComplete="name" placeholder="Enter Your Full Name" className={`${MODAL_INPUT} text-[15px] min-h-[44px] rounded-[10px] bg-white border-slate-200 shadow-sm focus:ring-2`} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <PhoneInput id="webinar-phone" name="phone" value={formData.phone} onChange={handleChange} required inputMode="numeric" autoComplete="tel" placeholder="Mobile Number" className={`${MODAL_INPUT} text-[15px] min-h-[44px] rounded-[10px] bg-white border-slate-200 shadow-sm focus:ring-2`} />
                    </div>
                  </div>
                  ...
                  <div className="pt-1 sm:col-span-2">
                    <button type="submit" className={`${WB_CTA} w-full justify-center min-h-[48px] rounded-[10px] !text-[14px] shadow-lg shadow-red-500/25 transition-transform hover:-translate-y-1`} disabled={isSubmitting}>
                      {isSubmitting ? 'Processing...' : 'Proceed to Pay Rs 499'}
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-1 pt-0.5 text-center text-slate-500 sm:col-span-2">
                    ...
                    <a href="https://wa.me/919005575577" target="_blank" rel="noopener noreferrer" className="m-0 text-[11px] font-bold text-green-600 hover:underline hover:text-green-700 mt-1 transition-colors">
