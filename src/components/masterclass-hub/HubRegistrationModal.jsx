...
function HubRegistrationModal({ isOpen, onClose, selectedClass, formData, handleChange, handleSubmit, isSubmitting }) {
            ...
            <div className="grid w-full grid-cols-1 md:grid-cols-[0.82fr_1.18fr] md:min-h-[252px] lg:min-h-[280px]">
              ...
              <div className="hidden flex-col justify-center border-t border-white/10 bg-[#3B2261] px-5 py-4 text-white md:order-1 md:flex md:border-r md:border-t-0 md:border-white/10 md:px-6 md:py-5 lg:px-7 lg:py-5">
                ...
                <h4 className={`${TYPE.h2} !m-0 !text-white !leading-[1.08] text-[21px] drop-shadow-md sm:text-[24px] md:text-[28px] lg:text-[31px]`}>
                  2-Day {selectedClass.title} <span className={`${WB_HIGHLIGHT} ml-1 inline-block`}>Masterclass</span>
                </h4>
              </div>
            </div>
            ...
            <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-slate-50">
              <div className="flex-1 overflow-y-auto p-3 sm:p-5">
                <div className="mb-3 rounded-[18px] border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
                  <h3 id="hub-modal-title" className={`${TYPE.h3} mb-2 flex items-center gap-2 text-[18px] text-slate-800 sm:text-[20px]`}>
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#3B2261] text-[11px] text-white shadow-sm">
                      <i className="fas fa-ticket-alt" />
                    </span>
                    2-Day {selectedClass.title} Masterclass
                  </h3>
                ...
                <div className="mb-3">
                  <ul className="m-0 grid gap-1.5 p-0 sm:grid-cols-3">
                    {selectedClass.highlights.map((point) => (
                ...
                <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <label htmlFor="hub-name" className={`${MODAL_LABEL} !mb-1 !text-[11px]`}>Full Name</label>
                    <input
                      id="hub-name"
                      type="text"
                      name="name"
                  ...
                  <div>
                    <label htmlFor="hub-email" className={`${MODAL_LABEL} !mb-1 !text-[11px]`}>Email Address</label>
                    <input
                      id="hub-email"
                      type="email"
                      name="email"
                  ...
                  <div>
                    <label htmlFor="hub-phone" className={`${MODAL_LABEL} !mb-1 !text-[11px]`}>Phone (WhatsApp)</label>
                  ...
                  <div>
                    <label htmlFor="hub-city" className={`${MODAL_LABEL} !mb-1 !text-[11px]`}>City</label>
                    <input
                      id="hub-city"
                      type="text"
                      name="city"
...
export default HubRegistrationModal;
