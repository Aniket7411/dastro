...
const CONTENT = {
  ...
  messages: [
    { name: 'Pooja', phone: '***** 4471', date: '2 Aug', chat: [{ side: 'them', text: 'Maam aaj ka session bahut accha tha. Practice wala part maine notebook mein likh liya. Kal ka wait kar rahi hoon.', time: '8:14 pm' }] },
    { name: 'Ritu', phone: '***** 9032', date: '2 Aug', chat: [{ side: 'them', text: 'Recording 2 baar dekhi. Har baar kuch naya samajh aata hai 🙏', time: '9:02 pm' }] },
    { name: 'Deepak', phone: '***** 7756', date: '3 Aug', chat: [{ side: 'them', text: 'Honestly ₹499 mein itna expect nahi kiya tha. Recording bhi mil gayi aur PDF notes bhi.', time: '7:31 pm' }] },
    { name: 'Manoj', phone: '***** 5519', date: '3 Aug', chat: [{ side: 'them', text: 'Zoom link time par mil gaya, koi problem nahi hui. DS Astro ki support team ka response fast tha.', time: '10:12 pm' }] },
    { name: 'Khushi', phone: '***** 2604', date: '4 Aug', chat: [{ side: 'them', text: 'PDF notes ke liye thank you. Simple English mein hai isliye padhne mein aasani hui.', time: '9:18 am' }] },
    { name: 'Anjali', phone: '***** 1188', date: '4 Aug', chat: [{ side: 'them', text: 'Meri beti bhi saath baithkar dekh rahi thi, use bhi interest aa gaya 😊', time: '1:44 pm' }] },
    { name: 'Sunita', phone: '***** 6142', date: '4 Aug', chat: [{ side: 'them', text: 'Damini Maam ka padhane ka tarika bahut simple hai. Ek baar mein samajh aa jaata hai.', time: '6:29 pm' }] },
    { name: 'Rakesh', phone: '***** 3390', date: '5 Aug', chat: [{ side: 'them', text: 'Mujhe laga tha ye sab bahut complicated hoga. Do din mein confidence aa gaya.', time: '9:55 pm' }] },
    { name: 'Neha', phone: '***** 4008', date: '5 Aug', chat: [{ side: 'them', text: 'Maam live course ki details bhej dijiye please. Aage seekhna hai.', time: '3:21 pm' }] },
    { name: 'Vikas', phone: '***** 7263', date: '5 Aug', chat: [{ side: 'them', text: 'Team ne call karke batch confirm kiya tha, bilkul waise hi hua. Setup professional hai.', time: '7:08 pm' }] },
    { name: 'Priya', phone: '***** 9471', date: '6 Aug', chat: [{ side: 'them', text: 'Class ke baad group mein sawal poocha tha, Maam ne jawab bhi diya. Yehi acchi baat hai.', time: '11:40 am' }] },
    { name: 'Ravi', phone: '***** 8827', date: '6 Aug', chat: [{ side: 'them', text: 'I joined out of curiosity, but now I want to do the full live course. Please share the details.', time: '8:47 pm' }] },
    { name: 'Meena', phone: '***** 1234', date: '6 Aug', chat: [{ side: 'them', text: 'Papa ke liye bhi join karwana chahti hoon, unko ye sab bahut pasand hai.', time: '9:00 pm' }] },
    { name: 'Sanjay', phone: '***** 5678', date: '7 Aug', chat: [{ side: 'them', text: 'Do ghante kaise nikal gaye pata hi nahi chala. Live demo wala part sabse accha laga.', time: '10:00 am' }] },
    { name: 'Kavita', phone: '***** 9012', date: '7 Aug', chat: [{ side: 'them', text: 'Maam ne mera example live liya tha class mein. Ghar mein sabko dikhaya 😄', time: '11:30 am' }] },
  ],
};
  ...
  return (
    <section className="ds-proof" aria-labelledby="proof-head">
      ...
      <div className="ds-videos">
        <div className="ds-wrap">
          <div className="ds-block-label"><h3>Video reviews</h3><span className="ds-count">{CONTENT.videos.length} reviews</span><span className="ds-hint">Auto-playing</span></div>
          <p className="ds-sound"><svg viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3a4.5 4.5 0 0 0-2.5-4v8a4.5 4.5 0 0 0 2.5-4z" /></svg>Tap a card to play. Hover to pause.</p>
        </div>
        <div className="ds-rail-outer">
          <div
            className="ds-rail ds-rail-auto"
            ref={railRef}
            role="list"
            aria-label="Student video reviews"
          >
            <div className={`ds-rail-track${isMarqueePaused ? ' is-paused' : ''}`}>
              {[...CONTENT.videos, ...CONTENT.videos].map((video, index) => (
                <VideoCard
                  key={`${video.name}-${index}`}
                  video={video}
                  isActive={playingIndex === index}
                  onPlay={() => { setPlayingIndex(index); setIsMarqueePaused(true); }}
                  onStop={() => { setPlayingIndex(null); setIsMarqueePaused(false); }}
                />
              ))}
            </div>
          </div>
      ...
      <div className="ds-cta"><div className="ds-cta-in"><p>Aap bhi seekhiye <span>2 din, live on Zoom</span></p><button className="ds-btn-cta" type="button" onClick={onJoinNow}>Join Masterclass <span className="price">₹499</span><span className="strike">₹1,999</span></button></div></div>
...
/* Auto-moving video carousel */
.ds-rail.ds-rail-auto{overflow-x:hidden;scroll-snap-type:none;padding:6px 4px 24px;-webkit-mask-image:linear-gradient(to right,transparent 0,#000 3%,#000 97%,transparent 100%);mask-image:linear-gradient(to right,transparent 0,#000 3%,#000 97%,transparent 100%)}
.ds-rail-track{display:flex;gap:14px;width:max-content;will-change:transform;animation:dsRailDrift 48s linear infinite}
.ds-rail-track.is-paused,.ds-rail.ds-rail-auto:hover .ds-rail-track{animation-play-state:paused}
.ds-rail.ds-rail-auto .ds-vcard{scroll-snap-align:none}
@keyframes dsRailDrift{from{transform:translateX(0)}to{transform:translateX(-50%)}}
@media(min-width:860px){.ds-rail.ds-rail-auto{padding-left:24px;padding-right:24px}}
@media(prefers-reduced-motion:reduce){.ds-rail-track{animation:none}}
`;
